<div class="card card-action mb-12">
    <div class="card-header">
        ...
        <div class="card-header-elements ms-auto">
            <button type="button" class="btn btn-success" data-bs-toggle="modal" data-bs-target="#modal_add">
                <span class="tf-icon ti ti-plus ti-xs me-1"></span>Add Kontrak
            </button>
            <button type="button" class="btn btn-warning" data-bs-toggle="modal" data-bs-target="#modal_import">
                <span class="tf-icon ti ti-plus ti-xs me-1"></span>Import Kontrak
            </button>
            <a href="<?= base_url('C_Kontrak/ExportKontrakRinci') ?>" class="btn btn-primary">
                ...
                <table id="table" class="table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td> <?= html_escape($d->tahun_anggaran) ?> </td>
                            <td style="text-align: center">
                                <?= html_escape($d->is_murni) == 1 ? "MURNI" : "LUNCURAN"; ?></td>
                            <td> <?= html_escape($d->no_skki) ?> </td>
                            ...
                            <td>
                                <button onclick="getMaterialKontrak('<?= html_escape($d->no_kontrak); ?>')"
                                    class="btn btn-secondary btn-sm" data-bs-toggle="modal"
                                    data-bs-target="#kt_modal_material">
                                    <?= html_escape($d->material) ?>
                                </button>
                            </td>
                            <td style="text-align: right;" class="text-nowrap">
                                <button onclick="getDetailNilai('<?= html_escape($d->no_kontrak); ?>')"
                                    class="btn btn-secondary btn-sm" data-bs-toggle="modal"
                                    data-bs-target="#kt_modal_nilai">
                                    <?php
                            ...
                            <td> <?= html_escape($d->is_using_jm) == 0 ? "TANPA BANK GARANSI" : html_escape($d->nomor_bae) ?>
                            </td>
                            <td> <?= html_escape($d->bae_awal) ?> </td>
                            <td> <?= html_escape($d->bae_akhir) ?> </td>
                            <td style="text-align: center">
                                <?php if (!empty(html_escape($d->file_kr))) { ?>
                                <a href="<?= base_url() . html_escape($d->file_kr_location) . '/' . html_escape($d->file_kr)  ?>"
                                    class="btn btn-text-danger btn-hover-light-danger btn-sm" target="_blank"> <i
                                        class="fa fa-file-pdf"></i> PDF
                                    <?php } ?>
                            </td>
                            <td>
                                <?php if (!empty(html_escape($d->file_bae))) { ?>
                                <a href="<?= base_url() . html_escape($d->file_bae_location) . html_escape($d->file_bae)  ?>"
                                    class="btn btn-text-danger btn-hover-light-danger btn-sm" target="_blank"> <i
                                        class="fa fa-file-pdf"></i> PDF
                                    <?php } ?>
                            </td>
                            ...
                            <td><?php
                                    if (html_escape($d->status_kontrak) == "SELESAI") {
                                        echo "<button class='btn btn-primary btn-sm'>SELESAI</button>";
                                    } else {
                                        if (html_escape($d->status_kirim) == 'SELESAI KIRIM') {
                                            echo "<button class='btn btn-primary btn-sm'>IN PROGRESS</button>";
                                        } else {
                                            if (html_escape($d->status_kontrak) == "IN PROGRESS") {
                                                echo "<button class='btn btn-primary btn-sm'>html_escape($d->status_kontrak)</button>";
                                            } else {
                                                echo "<button class='btn btn-danger btn-sm'>html_escape($d->status_kontrak)</button>";
                                            }
                                        }
                                    } ?>
                            </td>
                            <td>
                                <?php if (html_escape($d->status_kirim) == "BELUM KIRIM") { ?>
                                <button class='btn btn-danger btn-sm'><?= html_escape($d->status_kirim) ?></button>
                                <?php } else if (html_escape($d->status_kirim) == "PROSES KIRIM") { ?>
                                <button class='btn btn-warning btn-sm'><?= html_escape($d->status_kirim) ?></button>
                                <?php } else { ?>
                                <button class='btn btn-primary btn-sm'><?= html_escape($d->status_kirim) ?></button>
                                <?php } ?>
                            </td>
                            <td>
                                <?= html_escape($d->status_bayar) == "BELUM BAYAR"
                                        ?   "<button class='btn btn-danger btn-sm'>" . html_escape($d->status_bayar) . "</button>"
                                        :   "<button class='btn btn-primary btn-sm'>" . html_escape($d->status_bayar) . "</button>"; ?>
                            </td>
                            <td>
                                <div class="btn-group" role="group" aria-label="Basic example">
                                    <button class="btn btn-outline-secondary btn-sm  waves-effect waves-light"
                                        onclick="editdata('<?= html_escape($d->id) ?>')" data-bs-toggle="modal"
                                        data-bs-target="#modal_edit"><i class="fa fa-pencil"></i></button>
                                    <button class="btn btn-outline-secondary btn-sm  waves-effect waves-light"
                                        onclick="delete_data('<?= html_escape($d->id) ?>', '<?= html_escape($d->no_kontrak) ?>')"><i
                    ...
                    <tfoot>
                        <tr style="background-color:#008B8B">
                            <td style="text-align: center;" colspan="11"><b style="color:white"> TOTAL NILAI KONTRAK
                                </b> </td>
                            <td style="text-align: right; color:white;"> <b>
                                    <?= number_format($totalKontrak, 0, ',', '.'); ?> </b> </td>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="<?= base_url() ?>C_Kontrak/Save" method="POST" class="form" enctype="multipart/form-data">
                        ...
                        <div class="table-responsive">
                            <table id="table" class="table table-striped table-bordered tblMaterial"
                                ...
                                <thead>
                                    <tr style="background-color: #008B8B;">
                                        <th
                                            style="text-align: center; vertical-align: middle; color: white; width: 20%;">
                                            UNIT </th>
                                        <th
                                    ...
                                    <tr id="clone">
                                        <td style="text-align: center;">
                                            <select name='unit_tujuan_id[]' id='unit_tujuan_id[]'
                                                class='form-select select2 material' data-dropdown-parent='#modal_add'
                                                data-control='select2' data-placeholder='Pilih Unit' required>
                                                <option></option>
                                        ...
                                        <td style="text-align: center;">
                                            <select name='material[]' id='material[]'
                                                class='form-select select2 material' data-dropdown-parent='#modal_add'
                                                data-control='select2' data-placeholder='Pilih Material' required">
                                                <option></option>
                    ...
                    <div class="row mb-5">
                        <div class="col-md-12">
                            <button type='button' class='btn btn-success' onclick='duplicateRow()'>TAMBAH</button>
                        </div>
                    </div>
                    <div class="row mb-5">
                        <div class="col-md-6">
                            ...
                            <input type="text" name="nilai_kontrak" id="nilai_kontrak" autocomplete="off"
                                ...
                                value="<?php if (isset($kontrak)) {
                                                                                                                                                                                        echo $kontrak[0]->nilai_kontrak;
                                                                                                                                                                                    } ?>"
...
<div class="modal fade" id="modal_import" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <form action="<?= base_url() ?>C_Kontrak/ImportSave" method="POST" class="form"
                ...
                <div class="modal-body scroll-y m-5">
                    <div class="col">
                        ...
                        <br />
                        <a href="<?= base_url() ?>data_uploads/kontrak/format/FormatKontrakRinci 2024-09-11.xlsx"

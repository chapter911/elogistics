<div class="card card-action mb-12">
    <div class="card-header">
        <h5 class="card-action-title mb-0">Keandalan</h5>
    </div>
    <div class="collapse p-5 show">
        <form class="form" action="<?= base_url(); ?>C_Keandalan/exportIndex" method="POST">
                ...
                <table id="table" class="table">
                    <thead>
                        <tr style="background-color: #008B8B">
                            ...
                            <th style="text-align: center; vertical-align: middle; color: white;"> PELANGGAN / LOKASI </th>
...
<div class="modal fade" id="add_data" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-fullscreen p-9" role="document">
        <form id="add_data_form" class="form" action="<?= base_url(); ?>C_Keandalan/Save" method="POST">
            <input type="hidden" name="<?=$this->security->get_csrf_token_name();?>" value="<?=$this->security->get_csrf_hash();?>">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="modalCenterTitle">Pembuatan Keandalan Baru</h5>
                ...
                <div class="modal-body">
                    ...
                    <div class="row mb-5">
                        <div class="col-md-6">
                            ...
                            <input type="text" class="form-control" name="pelanggan" id="pelanggan" placeholder="" value="" autocomplete="off" />
                        </div>
                    </div>
                    <div class="row mb-5">
                        <div class="col-md-12">
                            ...
                            <div class="table-responsive">
                                <table id="table-insert-material" class="table table-striped table-bordered tblMaterial" style="width: 100%">
                                    <thead>
                                        <tr style="background-color: #008B8B;">
                                            <th style="text-align: center; vertical-align: middle; color: white; width: 5%;"> # </th>
                                            <th style="text-align: center; vertical-align: middle; color: white; width: 30%;"> MATERIAL </th>
                                            <th style="text-align: center; vertical-align: middle; color: white; width: 5%;"> VOLUME </th>
                                            <th style="text-align: center; vertical-align: middle; color: white; width: 10%;"> RASIO CT </th>
                                            <th style="text-align: center; vertical-align: middle; color: white; width: 10%;"> KETERANGAN </th>
                    ...
                    <div class="row mb-4">
                        <div class="col">
                            <button type="button" class="btn btn-primary" onclick="insertRow();">Tambah Material</button>
                        </div>
                    </div>
                </div>
...
<div class="modal fade" id="modal_update" tabindex="-1">
    <div class="modal-dialog modal-xl">
        <div class="modal-content modal-rounded">
            ...
            <div class="modal-body scroll-y m-5">
                <form class="form" action="<?= base_url() ?>C_Keandalan/update_approved_volume" method="POST">
                    <input type="hidden" name="<?=$this->security->get_csrf_token_name();?>" value="<?=$this->security->get_csrf_hash();?>">
                    <input type="hidden" name="id_keandalan" id="id_keandalan" class="form-control mb-3 mb-lg-0"
                        required />
                    <div class="table-responsive" id="updateMaterialKeandalan"></div>
...
$(document).ready(function() {
    table = $("#table").DataTable({
        ...
        "pageLength": 25,
        "ajax": {
            "url": "<?= base_url() ?>C_Keandalan/ajaxIndex",
            "type": "post",
            "beforeSend": function() {
                Swal.fire({
                    title: 'Mohon Tunggu',
...
function getMaterialKeandalan(id) {
    $.ajax({
        url: "<?= base_url() ?>C_Keandalan/materialKeandalan/" + id,
        type: "GET",
        success: function(d) {
...
function updateKeandalan(id) {
    $.ajax({
        ...
        success: function(d) {
            ...
            $("#id_keandalan").val(id);
            $("#updateMaterialKeandalan").html(d);
...
function deleteKeandalan(id) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: "<?= base_url() ?>C_Keandalan/delete",
                type: "POST",
                data: {
                    id: id,
...
function setRasio(data) {
    ...
    $.ajax({
        url: "<?= base_url() ?>C_Keandalan/cekUsingRatio",
        type: "POST",
        data: {

...
class C_Keandalan extends CI_Controller {
	...
	public function index(){
        ...
        $this->template->display('keandalan/index', $data);
	}
    ...
    function queryIndex(){
        ...
        $query = "SELECT * FROM vw_trn_keandalan_hdr $where";
    ...
    public function ajaxIndex(){
        ...
        foreach ($hasil as $h) {
            ...
            $row[] = "<button onclick='getMaterialKeandalan(" . html_escape($h->id) . ")' class='btn btn-secondary btn-sm'>MATERIAL</button>";
            ...
            if(html_escape($h->is_selesai) == 0){
                $button .= "<button onclick=\"updateKeandalan(" . html_escape($h->id) . ")\" class='btn btn-outline-secondary btn-sm'><i class=\"fa fa-pencil\"></i></button>";
            }

            if($this->session->userdata('delete') == 1){
                $button .= "<button onclick=\"deleteKeandalan(" . html_escape($h->id) . ")\" class='btn btn-outline-secondary btn-sm'><i class=\"fa fa-trash\"></i></button>";
            }
    ...
    function exportIndex(){
        ...
        header('Content-Disposition: attachment;filename="Dashboard_Keandalan_'.$date.'.xlsx"');
        header('Cache-Control: max-age=0');
        $writer->save('php://output');
    }

    function materialKeandalan(){
        $id = $this->uri->segment(3);
        $data['data'] = $this->M_AllFunction->Where("vw_material_keandalan", "keandalan_id = '$id'");
        $this->load->view('keandalan/materialKeandalan', $data);
    }
    ...
    public function Save(){
        ...
        $id = $this->M_AllFunction->InsertGetId('trn_keandalan_hdr', $data);
        ...
        $this->M_AllFunction->InsertBatch('trn_keandalan_dtl', $datadetail);

        redirect("C_Keandalan");
    }

    function update_approved_volume(){
        $id = $this->input->post("id_keandalan", true);
        ...
        for ($i = 0; $i < count($this->input->post('material', true)); $i++) {
            ...
            $this->M_AllFunction->Update("trn_keandalan_dtl", $data, "keandalan_id = '$id' AND material_id = '$material_id'");
        }
        if(isset($_POST['is_selesai'])){
            $this->M_AllFunction->Update('trn_keandalan_hdr', array("is_selesai" => '1'), "id = '$id'");
        }
    ...
    function delete(){
        ...
        $this->M_AllFunction->Delete("trn_keandalan_hdr", "id = '$id'");
        $this->M_AllFunction->Delete("trn_keandalan_dtl", "keandalan_id = '$id'");
        echo "success";
    }
}